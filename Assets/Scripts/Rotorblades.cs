...
public class Rotorblades : MonoBehaviour {
	...
	private static float w = 0.0001f; //Angular velocity, with only the magnitude represented here for its pseudo-vector. Direction is always "up".
	...
	public class Rotorblade : MonoBehaviour {
		...
		//A "slice" sitting on one rotorblade, next to other "slices"
	...
	void Update () {
		...
		Vector3 rotorSpin = spinner.spinRotor (w);
		var rotor = target.transform.Find ("Rotor_Control");

		rotor.transform.Rotate (rotorSpin);

		//Calculating the forces generated by virtual rotorblades
		float forceMag = 0.0f;
		for (int i = 0; i < rotorblades.Count; ++i) {
